...
from ioi.Code.model.group_pointcloud import FeatureNet
from ioi.Code.model.rpn import MiddleAndRPN
...
class RPN3D(object):
    ...
    def predict_step(self, INPUT_DATA_DIR, session, data, summary=False):
        # input:
        #     (N) tag
        ...
        batch_gt_boxes3d = label_to_gt_box3d(label,
                                             cls=self.cls,
                                             coordinate='lidar')
        print('predict', tag)
        input_feed = {}
        ...
        if summary:
            ...
            cur_tag = tag[0]
            P, Tr, R = load_calib( os.path.join(INPUT_DATA_DIR[0]) )
        ...
        for i in range(len(img)):
            ...
            front_image = draw_lidar_box3d_on_image(img[i],
                                                    ret_box3d[i],
                                                    ret_score[i],
                                                    batch_gt_boxes3d[i],
                                                    P2=P,
            ...
            bird_view = lidar_to_bird_view_img(lidar[i],
                                               factor=cfg.BV_LOG_FACTOR)
                                                 
            bird_view = draw_lidar_box3d_on_birdview(bird_view,
                                                     ret_box3d[i],
                                                     ret_score[i],
                                                     batch_gt_boxes3d[i],
                                                     factor=cfg.BV_LOG_FACTOR,
