...
from ioi.Code.utils.data_aug import aug_data
from ioi.Code.utils.preprocess import process_pointcloud

class Processor:
    def __init__(self, data_tag, f_rgb, f_lidar, f_label, INPUT_DATA_DIR, aug, is_testset):
    ...
    def __call__(self,load_index):
        if self.aug:
            ret = aug_data(self.data_tag[load_index], self.INPUT_DATA_DIR)
        else:
...
def iterate_data(INPUT_DATA_DIR, shuffle=False, aug=False, is_testset=False, batch_size=1, multi_gpu_sum=1):
    f_rgb = glob.glob(INPUT_DATA_DIR[1])
    f_label = glob.glob(INPUT_DATA_DIR[2])
    f_lidar = glob.glob(INPUT_DATA_DIR[3])
    f_rgb.sort()
    f_lidar.sort()
    f_label.sort()
    ...
    proc=Processor(data_tag, f_rgb, f_lidar, f_label, INPUT_DATA_DIR, aug, is_testset)
...
def sample_test_data(INPUT_DATA_DIR, batch_size=1, multi_gpu_sum=1):
    f_rgb = glob.glob(INPUT_DATA_DIR[1])
    f_label = glob.glob(INPUT_DATA_DIR[2])
    f_lidar = glob.glob(INPUT_DATA_DIR[3])
    f_rgb.sort()
