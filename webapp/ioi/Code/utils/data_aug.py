...
from ioi.Code.utils.utils import *
from ioi.Code.utils.preprocess import *

object_dir = './data/object'


def aug_data(tag, INPUT_DATA_DIR):
    np.random.seed()
    rgb = cv2.resize(cv2.imread(INPUT_DATA_DIR[1]), (cfg.IMAGE_WIDTH, cfg.IMAGE_HEIGHT))
    lidar = np.fromfile(INPUT_DATA_DIR[3], dtype=np.float32).reshape(-1, 4)
    label = np.array([line for line in open(INPUT_DATA_DIR[2], 'r').readlines()])  # (N')
